...
class HighlightView {
    ...
    private static final int DEFAULT_HIGHLIGHT_COLOR = Color.GRAY;
    private static final float HANDLE_RADIUS_DP = 0f;
    private static final float OUTLINE_DP = 2f;
    private static final int lineH = 200;
    private static final int lineW = 350;
    ...
    protected void draw(Canvas canvas) {
        ...
        if (!hasFocus()) {
            ...
            Log.e(drawRect.bottom + " " + drawRect.left + " " + drawRect.right + " " + drawRect.top);

            canvas.drawPath(path, outlinePaint);
            for (int i = drawRect.left + lineH; i < drawRect.right; i = i + lineH) {
                canvas.drawLine(i, drawRect.top, i, drawRect.bottom, linePaint);
            }
            for (int i = drawRect.top + lineW; i < drawRect.bottom; i = i + lineW) {
                canvas.drawLine(drawRect.left, i, drawRect.right, i, linePaint);
            }
